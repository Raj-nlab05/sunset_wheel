...
MUSIC_FILE = "rexlambo-falling-in-love.mp3"
...
def detect_dip(result, side):
    ...
    for lm_list, handed in zip(result.hand_landmarks, result.handedness):
        label = handed[0].category_name  # "Left" or "Right" from MediaPipe
        # MediaPipe labels are mirrored after flip, so Left=left hand, Right=right hand
...
def get_slice_index(px, py):
    ...
    if dist < INNER_R or dist > OUTER_R + 40:   # +40 so popped slice still registers
        return -1
...
def slice_points(i, extra_r=0):
    ...
    for s in range(steps + 1):
        ...
        pts.append((CENTER_X + math.cos(a) * (OUTER_R + extra_r),
...
def draw_wheel():
    for i in range(N_SLICES):
        ...
        # ── hover: bright white outline ──
        if is_hover:
            pygame.draw.polygon(screen, WHITE_SOFT, pts, 3)
        elif is_target:
            # pulsing orange outline for target
...
def handle_hit(idx):
    global active_idx, score, combo, target_idx
...
def draw_hud():
    ...
    # key hints at bottom
    hints = "SPACE: play/pause   M: toggle mode   S: summary   ESC: quit"
...
def draw_reaction_summary():
    screen.fill(BG_COLOR)
    draw_text_c(screen, "REACTION SUMMARY", font_xl, SOFT_YELLOW, WIN_W//2, 100)
    total = reaction_hits + reaction_misses
    acc   = (reaction_hits / total * 100) if total > 0 else 0
    y = 200
    draw_text_c(screen, f"Hits: {reaction_hits}",       font_lg, PEACH,      WIN_W//2, y); y += 50
    draw_text_c(screen, f"Misses: {reaction_misses}",   font_lg, WARM_PINK,  WIN_W//2, y); y += 50
    draw_text_c(screen, f"Accuracy: {acc:.1f}%",        font_lg, LAVENDER,   WIN_W//2, y); y += 50
    if best_reaction is not None:
        draw_text_c(screen, f"Best: {int(best_reaction)} ms",    font_lg, SOFT_ORANGE, WIN_W//2, y); y += 50
        draw_text_c(screen, f"Average: {int(avg_reaction)} ms",  font_lg, WHITE_SOFT,  WIN_W//2, y)
...
def main():
    ...
    while True:
        ...
        left_dip  = detect_dip(hand_data_raw, "left")
        right_dip = detect_dip(hand_data_raw, "right")
